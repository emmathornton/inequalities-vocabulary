...
language_descriptives = lang_descriptives1 %>% 
  ...
  add_row(Indicator = "Wealth", .before = 14) %>% 
  add_row(Indicator = "Occupational Status", .before = 20) %>% 
  add_row(Indicator = "Relative Neighbourhood Deprivation", .before = 25)
language_descriptives [is.na(language_descriptives )] <- " "
...
language_sesDescriptives <- language_descriptives %>% 
  ...
  add_header_row(values = c(" ", "Proportion (%) or Mean(±SD) [95% CIs]" ), colwidths = c(1,4)) %>% 
  font(fontname = "Times New Roman", part="header") %>% 
  align(align="center", part="header") %>% 
 ...
 print(language_sesDescriptives, preview = "docx") 
